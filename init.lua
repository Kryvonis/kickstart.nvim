...
vim.api.nvim_create_autocmd('BufWinLeave', {
  ...
  pattern = '*',
  callback = function()
    if vim.bo.buftype == '' and vim.fn.expand '%' ~= '' then vim.cmd 'silent! mkview' end
  end,
})
...
vim.api.nvim_create_autocmd('BufWinEnter', {
  ...
  pattern = '*',
  callback = function()
    if vim.bo.buftype == '' and vim.fn.expand '%' ~= '' then vim.cmd 'silent! loadview' end
  end,
})
...
-- Save of C-s
vim.keymap.set({ 'n', 'i', 'v' }, '<C-s>', function() vim.cmd 'w' end, { noremap = true, silent = true, desc = 'Save File' })
...
vim.api.nvim_create_autocmd('FileType', {
  pattern = { 'json', 'jsonc' },
  callback = function()
    vim.schedule(function() vim.opt_local.foldmethod = 'indent' end)
  end,
})
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          if vim.bo.filetype == 'python' then
            map('<leader>po', function() vim.cmd '!poetry install' end, '[P]oetry Install')

            map('<leader>pr', function() vim.cmd '!poetry run python %' end, '[P]oetry [R]un current file')

            map('<leader>ps', function() vim.cmd '!poetry shell' end, '[P]oetry [S]hell')
            ...
            map('<leader>pld', function() vim.diagnostic.setloclist() end, '[P]yright [L]SP [D]iagnostics')
          end
      ...
      local servers = {
        ...
        pyright = {
          ...
          before_init = function(_, config)
            ...
            local path = {
              join = function(...) return vim.fs.joinpath(...) end,
              dirname = function(p) return vim.fs.dirname(p) end,
            }
            ...
            local function parse_pyproject_toml(file_path)
              ...
              if name_match then result.project_name = name_match end

              -- Parse [tool.pyright] section more carefully
              local pyright_section = content:match '%[tool%.pyright%](.-)%[tool%.'
              if not pyright_section then pyright_section = content:match '%[tool%.pyright%](.*)$' end
              ...
              if pyright_section then
                ...
                if version_match then result.pyright_config.pythonVersion = version_match end

                local missing_imports = pyright_section:match 'reportMissingImports%s*=%s*["\']?([^"\'%s]+)["\']?'
                if missing_imports then result.pyright_config.reportMissingImports = missing_imports end
              end
              ...
              if handle then
                while true do
                  ...
                  if type == 'directory' and not name:match '^%.' then
                    ...
                    if vim.fn.filereadable(init_file) == 1 then table.insert(src_paths, dir_path) end
                  end
                end
              end
                  ...
                  if venv_path and venv_path ~= '' then
                    ...
                    if vim.fn.isdirectory(venv_path) == 1 then
                      ...
                      for _, python_path in ipairs(python_paths) do
                        if vim.fn.executable(python_path) == 1 then return python_path, venv_path end
                      end
                    end
                  end
                end
            ...
            local function find_pyproject_toml(start_dir)
              ...
              for _, file_info in ipairs(pyproject_files) do
                ...
                if project_info and (project_info.pyright_config.include or file_info.is_root) then return file_info.path, file_info.dir, project_info end
              end

              -- Fallback to the first Poetry project found
              for _, file_info in ipairs(pyproject_files) do
                local project_info = parse_pyproject_toml(file_info.path)
                if project_info and project_info.is_poetry_project then return file_info.path, file_info.dir, project_info end
              end
            ...
            local function setup_poetry_project()
              ...
              if not pyproject_path or not project_info then return end
              ...
              if python_path then
                ...
                if venv_path then config.settings.python.venvPath = venv_path end
              end

              -- Apply pyproject.toml Pyright configuration
              if project_info.pyright_config.include then config.settings.python.analysis.include = project_info.pyright_config.include end

              if project_info.pyright_config.exclude then config.settings.python.analysis.exclude = project_info.pyright_config.exclude end

              if project_info.pyright_config.pythonVersion then config.settings.python.pythonVersion = project_info.pyright_config.pythonVersion end
              ...
              if project_info.pyright_config.include then
                for _, include_path in ipairs(project_info.pyright_config.include) do
                  ...
                  if vim.fn.isdirectory(full_path) == 1 then table.insert(extra_paths, full_path) end
                end
              end

              if #extra_paths > 0 then config.settings.python.analysis.extraPaths = extra_paths end

              -- Only show notification if there are issues
              if not python_path then
                vim.schedule(function() vim.notify('Poetry venv not found for: ' .. root_dir, vim.log.levels.WARN, { title = 'Pyright Setup' }) end)
              end
            end
